...
import dialogfilter from 'src/pages/cargas/transferencia/dialog-filter.vue'

// import ChangeMotorista from 'src/pages/operacional/coletas/trocarapida/cpn-change-motorista'

class CargasTransfers {
  constructor () {
    this.limpardados()
  ...
  async fetch (pLoadMore = false) {
    ...
    if (self.params.unidade ? self.params.unidade.id > 0 : false) {
      params[(self.params.origem === 'e') ? 'unidadeentradaid' : 'unidadesaidaid'] = self.params.unidade.id
    }
    ...
    let ret = await Vue.prototype.$axios.get('v1/cargatransfer', { params: params }).then(response => {
      ...
      if (data) {
        ...
        if (data.ok) {
          ...
          for (let index = 0; index < data.rows.length; index++) {
            ...
            let p = new CargaTransfer(element)
...
export default CargasTransfers
