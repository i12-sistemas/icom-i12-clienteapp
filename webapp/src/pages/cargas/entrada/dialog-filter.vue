      ...
      <q-card-section class="scroll q-pa-sm q-gutter-md" v-if="!loading" >
        <q-card class="my-card" bordered flat>
          ...
          <q-card-section class="q-pa-none q-ma-none">
            <div class="q-pa-md">
              <selectunidade outlined label="Unidade" v-model="filter.unidade" ref="txtunidade" />
            </div>
          </q-card-section>
        </q-card>
        <q-card class="my-card" bordered flat>
          ...
          <q-card-section class="q-pa-none q-ma-none">
            <div class="q-pa-md">
              <inputdata outlined v-model="filter.created_at" label="Data de abertura" stacklabel  />
            </div>
          </q-card-section>
        </q-card>
        <q-card class="my-card" bordered flat>
          ...
          <q-card-section class="q-pa-none q-ma-none">
            <div class="q-pa-md">
              <q-option-group v-model="filter.status" type="radio" :options="options_status" inline />
            </div>
          </q-card-section>
        </q-card>
        <q-card class="my-card" bordered flat>
          <q-card-section class="bg-grey-2 q-pa-sm text-subtitle2">Tipo de Entrada</q-card-section>
          <q-card-section class="q-pa-none q-ma-none">
            <div class="q-pa-md">
              <q-option-group v-model="filter.tipo" type="radio" :options="options_tipo" inline />
            </div>
          </q-card-section>
        </q-card>
        <q-card class="my-card" bordered flat>
...
export default {
  ...
  async mounted () {
    var app = this
    if (app.config) {
      app.filter.status = app.config.status
      app.filter.tipo = app.config.tipo
      app.filter.created_at = app.config.created_at
      app.filter.unidade = app.config.unidade
      app.filter.erros = app.config.erros
    }

    var opttipo = new CargaEntradaTipo('1')
    for (let index = 0; index < opttipo.options.length; index++) {
      const opt = opttipo.options[index]
      app.options_tipo.push({ value: opt.value, label: opt.desc })
    }
    app.options_tipo.push({ value: null, label: 'Nenhum' })

    var optstatus = new CargaEntradaStatus('1')
