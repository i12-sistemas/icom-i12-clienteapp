    ...
    <q-page>
        ...
        <div class="q-pa-sm"  v-if="!loading">
          <q-card class="full-width q-ma-sm" bordered flat>
            ...
            <q-card-section >
              <selectunidadeall outlined label="Unidade de Destino" v-model="dataset.unidadeentrada" ref="txtunidade" :clearable="false" />
            </q-card-section>
            <q-separator  spaced />
            <q-card-section >
...
import selectunidadeall from 'src/components/cnp-select-unidade'
  ...
  methods: {
    async actSave () {
      ...
      if (ret.ok) {
        ...
        app.$nextTick(() => {
          app.loading = true
          app.$router.push({ name: 'cargas.transferencias.edit', params: { id: app.dataset.id } })
          app.loading = false
        })
      } else {
