      ...
      <q-card-section class="scroll q-pa-sm q-gutter-md" v-if="!loading" >
        <q-card class="my-card" bordered flat>
          ...
          <q-card-section class="q-pa-none q-ma-none">
            <div class="q-pa-md">
              <selectunidade outlined label="Unidade de Saída" v-model="filter.unidade" ref="txtunidade" :clearable="true" />
            </div>
          </q-card-section>
        </q-card>
        <q-card class="my-card" bordered flat>
        ...
        <q-card class="my-card" bordered flat>
          <q-card-section class="bg-grey-2 q-pa-sm text-subtitle2">Erros no palete</q-card-section>
          <q-card-section class="q-pa-none q-ma-none">
            <div class="q-pa-md">
...
import { PaletesStatus } from 'src/mvc/models/enums/cargastypes'
export default {
  props: [
    'config'
  ],
  ...
  async mounted () {
    ...
    var optstatus = new PaletesStatus('1')
    for (let index = 0; index < optstatus.options.length; index++) {
      const opt = optstatus.options[index]
      app.options_status.push({ value: opt.value, label: opt.desc, color: opt.bgcolor })
    }
    app.loading = false
  },
  methods: {
