...
<q-layout view="hHh lpR fFf">
  <q-header reveal class="bg-primary text-white shadow-2">
    <q-toolbar>
      ...
      <q-btn round flat icon="add" :to="{ name: 'cargas.entregas.add' }" />
  ...
  <q-page-container class="bg-grey-2" >
    <q-page>
      ...
      <div class="full-width text-body q-pa-md" v-for="(item, key) in rows" :key="key" >
        <q-card class="my-card full-width" bordered flat v-ripple @click="$router.push({ name: 'cargas.entregas.edit', params: { id: item.id } })" >
          <q-card-section>
            <div class="row">
              <div class="col-12" >
                <div class="row">
                  ...
                  <div class="col-12 text-primary q-pa-sm rounded-borders q-mb-xs" :class="dataset.params.origem === 's' ? 'bg-grey-3' : 'bg-grey-1'">
                    ...
                    <div v-if="item.unidadesaida">
                      ...
                      <div>
                        ...
                        <span v-if="item.unidadesaida.endereco" class="text-weight-bold q-ml-xs">{{ item.unidadesaida.endereco.cidade.cidade }}</span>
                      </div>
                    </div>
                  </div>
                  <div class="col-12">
...
<script>
import CargasEntregas from 'src/mvc/collections/cargasentregas.js'
export default {
  components: {
  },
  directives: {
  ...
  props: ['label'],
  data: function () {
    let dataset = new CargasEntregas()
    return {
      dataset,
      rows: [],
      ativos: true,
